			...
			<form action="" method="post">
				<div class="row">
					...
					<div class="form-group col-md-12">
						<button type="submit"
						        class="btn btn-primary"><?php _e( 'Filtrera', 'digitalboard-textdomain' ); ?></button>
					</div>
			...
			<div class="list-group">
				<?php if ( $posts->have_posts() ): while ( $posts->have_posts() ): $posts->the_post(); ?>
					<a href="<?php echo get_permalink( $post->ID ); ?>"
					   class="list-group-item list-group-item-action"><strong><?php echo Digitalboard_Public::get_taxonomy_name( get_field( 'digitalboard_type', get_the_ID() ), 'digitalboard-notice' )->name; ?>
							: <?php echo $post->post_title; ?>
							, <?php echo get_field( 'digitalboard_date', $post->ID ); ?></strong><span
							class="read-more"><?php the_icon( 'arrow-right-circle' ) ?></span></a>
				<?php endwhile;
				else : ?>
